...
if($do == 'display') {
	...
	if(count($list) == 1 && $list[0]['uid'] && !empty($keyword)) {
		...
		foreach($list as &$li) {
			if(empty($li['email']) || (!empty($li['email']) && substr($li['email'], -6) == 'www.v-888.com' && strlen($li['email']) == 39)) {
				$li['email'] = '未完善';
			}
		}
		$status = 0;
...
if($do == 'modal') {
	if($_W['isajax']) {
		...
		if(empty($data['email']) || (!empty($data['email']) && substr($data['email'], -6) == 'www.v-888.com' && strlen($data['email']) == 39)) {
			$data['email'] = '未完善';
		}
