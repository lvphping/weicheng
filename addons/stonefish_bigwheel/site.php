...
class Stonefish_bigwheelModuleSite extends WeModuleSite {
    ...
    public function gethome() {
        global $_W;
        $articles = pdo_fetchall("select id,rid, title from " . tablename('stonefish_bigwheel_reply') . " where uniacid = '{$_W['uniacid']}'");
        if (!empty($articles)) {
    ...
    public function doMobileShare() {
        ...
        $acid = $_W['acid'];
        if (empty($acid)) {
            $acid = pdo_fetchcolumn("select share_acid from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
        }
        //print_r($_W);
        //exit;
        if (empty($from_user)) {
        ...
        if ($from_user != $fromuser) {
            $sharenumtype = pdo_fetchcolumn("select sharenumtype from " . tablename('stonefish_bigwheel_share') . " where uniacid = :uniacid and acid = :acid and rid = :rid order by `id` desc", array(':uniacid' => $uniacid, ':acid' => $acid, ':rid' => $rid));
            if ($sharenumtype == 0) {
                $sharedata = pdo_fetch("select id from " . tablename('stonefish_bigwheel_data') . " where rid = '" . $rid . "' and from_user = '" . $from_user . "' and uniacid = '" . $uniacid . "'  limit 1");
            } else {
                $sharedata = pdo_fetch("select id from " . tablename('stonefish_bigwheel_data') . " where rid = '" . $rid . "' and from_user = '" . $from_user . "' and fromuser = '" . $fromuser . "' and uniacid = '" . $uniacid . "'  limit 1");
            }
            //记录分享
            $insert = array(
                'rid' => $rid,
            ...
            if (empty($sharedata)) {
                pdo_insert('stonefish_bigwheel_data', $insert);
            }
            //记录分享
        }
    ...
    function get_share($uniacid, $rid, $fromuser, $title) {
        if (!empty($rid)) {
            //虚拟人数据配置
            $now = time();
            $reply = pdo_fetch("select xuninum_time,xuninumtime,xuninum,xuninuminitial,xuninumending from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
            if ($now - $reply['xuninum_time'] > $reply['xuninumtime']) {
                pdo_update('stonefish_bigwheel_reply', array('xuninum_time' => $now, 'xuninum' => $reply['xuninum'] + mt_rand($reply['xuninuminitial'], $reply['xuninumending'])), array('rid' => $rid));
            }
            //虚拟人数据配置
            $total = pdo_fetchcolumn("select xuninum+fansnum as total from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
        }
        if (!empty($fromuser)) {
            $realname = pdo_fetchcolumn("select realname from " . tablename('stonefish_bigwheel_fans') . " where uniacid= :uniacid and rid= :rid and from_user= :fromuser", array(':uniacid' => $uniacid, ':rid' => $rid, ':fromuser' => $fromuser));
        }
        $gifttitle = pdo_fetchcolumn("select description from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $uniacid . "' and rid = '" . $rid . "' and from_user='" . $fromuser . "' and status>0 and prize>0 order by prize asc");
    ...
    public function doMobileindex() {
        ...
        if ($reply['homepictime'] > 0) {
            if (empty($_COOKIE['stonefish_bigwheel_homepictime']) || $_COOKIE["stonefish_bigwheel_homepictime"] <= time()) {
                setcookie("stonefish_bigwheel_homepictime", mktime(23, 59, 59, date('d'), date('m'), date('Y')), mktime(23, 59, 59, date('d'), date('m'), date('Y')));
        ...
        if (empty($from_user)) {
            ...
            $fans = pdo_fetch("select * from " . tablename('stonefish_bigwheel_fans') . " where rid = '" . $rid . "' and fansID='" . $fansID . "' and from_user='" . $from_user . "'");
            if ($fans == false && $running == true) {
                ...
                $temp = pdo_insert('stonefish_bigwheel_fans', $insert);
            ...
            if ($reply['opportunity'] == 1) {
                ...
                if (empty($profile['mobile'])) {
                    message('还没有注册成为会员，无法进入抽奖', url('entry//member', array('m' => 'stonefish_member', 'url' => url('entry//index', array('m' => 'stonefish_bigwheel', 'rid' => $rid)))), 'error');
                    exit();
                }
            ...
            if ($share['sharenum'] > 0) {
                if ($share['sharetype']) {
                    $sharenum = pdo_fetchcolumn("select count(id) as dd from " . tablename('stonefish_bigwheel_data') . " where uniacid='" . $uniacid . "' and rid= '" . $rid . "' and fromuser='" . $from_user . "'");
            ...
            //中奖名单
            if ($reply['awardnum']) {
                $awardlist = pdo_fetchall("select * from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $uniacid . "' and rid = '" . $rid . "' and status>=1 group by fansID order by id desc limit " . $reply['awardnum']);
                foreach ($awardlist as $awardlists) {
                    $awardname = pdo_fetchcolumn("select realname from " . tablename('stonefish_bigwheel_fans') . " where uniacid='" . $uniacid . "' and rid = '" . $rid . "' and fansID='" . $awardlists['fansID'] . "'");
                    $awardz = pdo_fetchall("select * from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $uniacid . "' and rid = '" . $rid . "' and fansID='" . $awardlists['fansID'] . "' and status>=1 group by name order by id desc");
                    $awardx = '';
                    foreach ($awardz as $awards) {
                        $prizex = pdo_fetchcolumn("select COUNT(*) from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $uniacid . "' and rid = '" . $rid . "' and fansID='" . $awardlists['fansID'] . "' and status>=1 and name='" . $awards['name'] . "'");
                        if ($prizex == 1) {
                            $awardx .= $awards['name'] . '&nbsp;&nbsp;';
                        } else {
        ...
        $prizenum = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $uniacid . "' and rid= '" . $rid . "' and status>0");
        ...
        $sharetitle = empty($share['share_title']) ? '欢迎参加大转盘活动' : $share['share_title'];
        $sharedesc = empty($share['share_desc']) ? '亲，欢迎参加大转盘抽奖活动，祝您好运哦！！' : str_replace("\r\n", " ", $share['share_desc']);
    ...
    public function doMobileawardinfo() {
        ...
        $reply = pdo_fetch("select title,adpic,adpicurl,award_info,turntable,show_num from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
        if ($reply == false) {
            message('抱歉，活动已经结束，下次再来吧！', '', 'error');
        }
        //奖品
        $prize = pdo_fetchall("select prizetype,prizename,prizetotal,prizepic from " . tablename('stonefish_bigwheel_prize') . " where rid = :rid and turntable=:turntable order by `id` asc", array(':rid' => $rid, ':turntable' => $reply['turntable']));
    ...
    public function doMobileGift() {
        ...
        $reply = pdo_fetch("select title,adpic,adpicurl,opportunity,ticket_information from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
        if ($reply == false) {
            message('抱歉，活动已经结束，下次再来吧！', '', 'error');
        }
        //获得关键词
        ...
        //获得关键词
        //已兑奖项
        $award = pdo_fetchall("select name,description from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $_W['uniacid'] . "' and rid = '" . $rid . "' and fansID='" . $fansID . "' and status=2 group by name order by id desc");
        foreach ($award as $mid => $awards) {
            $award[$mid]['num'] = pdo_fetchcolumn("select COUNT(*) from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $_W['uniacid'] . "' and rid = '" . $rid . "' and fansID='" . $fansID . "' and status=2 and name='" . $awards['name'] . "'");
        }
        //未兑奖项
        $awardw = pdo_fetchall("select name,description from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $_W['uniacid'] . "' and rid = '" . $rid . "' and fansID='" . $fansID . "' and status=1 group by name order by id desc");
        foreach ($awardw as $mid => $awardws) {
            $awardw[$mid]['num'] = pdo_fetchcolumn("select COUNT(*) from " . tablename('stonefish_bigwheel_award') . " where uniacid='" . $_W['uniacid'] . "' and rid = '" . $rid . "' and fansID='" . $fansID . "' and status=1 and name='" . $awardws['name'] . "'");
        }
    ...
    public function doMobileRule() {
        ...
        $share = pdo_fetch("select share_txt,share_url from " . tablename('stonefish_bigwheel_share') . " where rid = :rid and acid = :acid", array(':rid' => $rid, ':acid' => $acid));
        $reply = pdo_fetch("select title,adpic,adpicurl from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
        if ($reply == false) {
            message('抱歉，活动已经结束，下次再来吧！', '', 'error');
        }
        //查询是否为关注用户
    ...
    public function doMobileGet_share() {
        ...
        //判断是否为关注用户
        $fans = pdo_fetch("select * from " . tablename('stonefish_bigwheel_fans') . " where rid = " . $rid . " and from_user='" . $from_user . "'");
        if ($fans == false) {
            //数据出错
            $data = array(
            ...
            //保存分享次数
            pdo_update('stonefish_bigwheel_fans', array('sharenum' => $fans['sharenum'] + 1, 'sharetime' => time()), array('id' => $fans['id']));
            $data = array(
                'msg' => '分享次数保存成功！',
                'success' => 1,
            );
    ...
    public function doMobileduijiang() {
        ...
        $fans = pdo_fetch("select id,mobile from " . tablename('stonefish_bigwheel_fans') . " where rid = " . $rid . " and fansID=" . $fansID . "");
        if ($fans == false) {
            $data = array(
                'success' => 0,
                'msg' => '兑奖数据错误！',
            ...
            if (!empty($business)) {
                $temp = pdo_update('stonefish_bigwheel_award', array('status' => 2), array('rid' => $rid, 'fansID' => $fansID, 'uniacid' => $_W['uniacid']));
                if ($temp === false) {
                    $data = array(
                        'success' => 0,
                        'msg' => '兑奖数据保存错误！',
                    ...
                    pdo_update('stonefish_bigwheel_fans', array('zhongjiang' => 2), array('id' => $fans['id']));
                    $data = array(
                        'success' => 1,
                        'msg' => '兑奖成功！',
                    );
    ...
    public function setfans() {
        ...
        if (empty($fansID) || empty($id))
            return;
        $fans = pdo_fetch("select * from " . tablename('stonefish_bigwheel_fans') . " where rid = " . $id . " and fansID=" . $fansID . "");
        $nowtime = mktime(0, 0, 0);
        if ($fans['last_time'] < $nowtime) {
            $fans['todaynum'] = 0;
        }
        ...
        pdo_update('stonefish_bigwheel_fans', $update, array('id' => $fans['id']));
    }
    ...
    public function doWebManage() {
        ...
        $params[':module'] = 'stonefish_bigwheel';
        ...
        if (!empty($list)) {
            foreach ($list as &$item) {
                ...
                $bigwheel = pdo_fetch("select fansnum, viewnum,starttime,endtime,isshow from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid ", array(':rid' => $item['id']));
    ...
    public function doWebFanslist() {
        ...
        foreach ($list as &$lists) {
            $lists['awardinfo'] = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_award') . "  where rid = :rid and fansID=:fansID", array(':rid' => $rid, ':fansID' => $lists['fansID']));
            $lists['share_num'] = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_data') . "  where rid = :rid and fromuser=:fromuser", array(':rid' => $rid, ':fromuser' => $lists['from_user']));
        }
        //中奖情况
        //一些参数的显示
        $num1 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_fans') . "  where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num2 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_fans') . "  where rid = :rid and uniacid=:uniacid and zhongjiang>0", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num3 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_fans') . "  where rid = :rid and uniacid=:uniacid and zhongjiang=1", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num4 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_fans') . "  where rid = :rid and uniacid=:uniacid and zhongjiang=2", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num5 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_fans') . "  where rid = :rid and uniacid=:uniacid and zhongjiang=0", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num6 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_fans') . "  where rid = :rid and uniacid=:uniacid and xuni=1", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        //一些参数的显示
    ...
    public function doWebEditbranchsave() {
        ...
        if ($uid) {
            ...
            message('修改抽奖次数成功', url('site/entry/branch', array('rid' => $rid, 'm' => 'stonefish_bigwheel')));
        } else {
    ...
    public function doWebAddaward() {
        ...
        if ($_W['isajax']) {
            ...
            $reply = pdo_fetch("select * from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
            //粉丝数据
            $data = pdo_fetch("select id, fansID, realname, mobile, uniacid  from " . tablename('stonefish_bigwheel_fans') . ' where id = :id and uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
            //奖品数据
            $awardlist = pdo_fetchall("select * from " . tablename('stonefish_bigwheel_prize') . ' where rid = :rid and uniacid = :uniacid and turntable=:turntable order by id asc', array(':uniacid' => $_W['uniacid'], ':rid' => $rid, ':turntable' => $reply['turntable']));
            //判断是否还有奖品
    ...
    public function doWebAwardfrom() {
        ...
        if ($_W['isajax']) {
            ...
            //粉丝数据
            $data = pdo_fetch("select id, fansID, realname, mobile, uniacid  from " . tablename('stonefish_bigwheel_fans') . ' where id = :id and uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
            $list = pdo_fetchall("select * from " . tablename('stonefish_bigwheel_award') . "  where rid = :rid and uniacid=:uniacid and fansID=:fansID order by id desc ", array(':uniacid' => $_W['uniacid'], ':rid' => $rid, ':fansID' => $data['fansID']));
    ...
    public function doWebUserinfo() {
        ...
        if ($_W['isajax']) {
            ...
            //粉丝数据
            if ($fansID) {
                $data = pdo_fetch("select * from " . tablename('stonefish_bigwheel_fans') . ' where fansID = :fansID and uniacid = :uniacid and rid = :rid', array(':uniacid' => $_W['uniacid'], ':fansID' => $fansID, ':rid' => $rid));
            } else {
                $data = pdo_fetch("select * from " . tablename('stonefish_bigwheel_fans') . ' where id = :id and uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
            }
    ...
    public function doWebSharelist() {
        ...
        if ($_W['isajax']) {
            ...
            $rid = intval($_GPC['rid']);
            //粉丝数据
            $data = pdo_fetch("select id, fansID, realname, mobile, from_user  from " . tablename('stonefish_bigwheel_fans') . ' where id = :id and uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
            $share = pdo_fetchall("select * from " . tablename('stonefish_bigwheel_data') . "  where rid = :rid and uniacid=:uniacid and fromuser=:fromuser order by id desc ", array(':uniacid' => $_W['uniacid'], ':rid' => $rid, ':fromuser' => $data['from_user']));
    ...
    public function doWebAddawardsave() {
        ...
        if (!$awardid) {
            message('必需选择奖品才能生效', url('site/entry/fanslist', array('rid' => $rid, 'm' => 'stonefish_bigwheel')), 'error');
        }
        if (!$rid) {
            message('系统出错', url('site/entry/fanslist', array('rid' => $rid, 'm' => 'stonefish_bigwheel')), 'error');
        }
        if ($uid) {
            //规则
            $reply = pdo_fetch("select * from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
            //粉丝数据
            $data = pdo_fetch("select id, fansID, from_user, awardnum  from " . tablename('stonefish_bigwheel_fans') . ' where id = :id and uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
            //添加中奖记录
            $sn = random(16);
            $award = pdo_fetch("select * from " . tablename('stonefish_bigwheel_prize') . "  where id=:id", array(':id' => $awardid));
            pdo_update('stonefish_bigwheel_prize', array('prizedraw' => $award['prizedraw'] + 1), array('id' => $awardid));
            $k = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_prize') . " where uniacid='" . $_W['uniacid'] . "' and rid= '" . $rid . "' and id<='" . $awardid . "' order by `id` asc");
            //保存sn到award中
            $insert = array(
                'uniacid' => $_W['uniacid'],
                'rid' => $rid,
            ...
            message('添加虚拟中奖成功', url('site/entry/fanslist', array('rid' => $rid, 'm' => 'stonefish_bigwheel')));
        } else {
            message('未找到指定用户', url('site/entry/fanslist', array('rid' => $rid, 'm' => 'stonefish_bigwheel')), 'error');
        }
    }
    ...
    public function doWebDeletefans() {
        global $_GPC, $_W;
        $rid = intval($_GPC['rid']);
        $reply = pdo_fetch("select id, fansnum from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        if (empty($reply)) {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
            ...
            if ($id == 0)
                continue;
            $fans = pdo_fetch("select fansID from " . tablename('stonefish_bigwheel_fans') . " where id = :id", array(':id' => $id));
            if (empty($fans)) {
                $this->webmessage('抱歉，选中的粉丝数据不存在！');
            }
            //删除粉丝中奖记录
            $fansaward = pdo_fetchall("select id,prizetype from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and uniacid=:uniacid and fansID=:fansID", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':fansID' => $fans['fansID']));
            foreach ($fansaward as $fansawards) {
                $prize = pdo_fetch("select id,prizedraw from " . tablename('stonefish_bigwheel_prize') . " where id = :id", array(':id' => $fansawards['prizetype']));
                pdo_update('stonefish_bigwheel_prize', array('prizedraw' => $prize['prizedraw'] - 1), array('id' => $fansawards['prizetype']));
                //查询奖品是否为虚拟积分，如果是则扣除相应的积分
                //查询奖品是否为虚拟积分，如果是则扣除相应的积分
                pdo_delete('stonefish_bigwheel_award', array('id' => $fansawards['id']));
            }
            //删除粉丝中奖记录
            //删除粉丝参与记录
            ...
            pdo_update('stonefish_bigwheel_reply', array('fansnum' => $reply['fansnum']), array('id' => $reply['id']));
            //减少参与记录			
        }
    ...
    public function doWebDeleteaward() {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
            ...
            if ($id == 0)
                continue;
            //删除奖品第一步先恢复到奖池中
            $award = pdo_fetch("select id,prizetype,fansID from " . tablename('stonefish_bigwheel_award') . " where id = :id", array(':id' => $id));
            $prize = pdo_fetch("select id,prizedraw from " . tablename('stonefish_bigwheel_prize') . " where id = :id", array(':id' => $award['prizetype']));
            pdo_update('stonefish_bigwheel_prize', array('prizedraw' => $prize['prizedraw'] - 1), array('id' => $award['prizetype']));
            //删除奖品第一步先恢复到奖池中
            //查询粉丝是否还有中奖记录，没有则需要改变粉丝状态
            $fansaward = pdo_fetch("select id from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and fansID = :fansID and uniacid=:uniacid and id!=:id", array(':rid' => $rid, ':fansID' => $award['fansID'], ':uniacid' => $_W['uniacid'], ':id' => $id));
            if (empty($fansaward)) {
                pdo_update('stonefish_bigwheel_fans', array('zhongjiang' => 0), array('fansID' => $award['fansID'], 'uniacid' => $_W['uniacid'], 'rid' => $rid));
            } else {
                $awardnum = pdo_fetchcolumn("select count(*) from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and uniacid=:uniacid and fansID=:fansID and id!=:id", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':fansID' => $award['fansID'], ':id' => $id));
                pdo_update('stonefish_bigwheel_fans', array('awardnum' => $awardnum, 'zhongjiang' => 1), array('fansID' => $award['fansID'], 'uniacid' => $_W['uniacid'], 'rid' => $rid));
            }
            //查询粉丝是否还有中奖记录，没有则需要改变粉丝状态
            //查询奖品是否为虚拟积分，如果是则扣除相应的积分
            //查询奖品是否为虚拟积分，如果是则扣除相应的积分
            //删除粉丝中奖记录
            pdo_delete('stonefish_bigwheel_award', array('id' => $id));
            //删除粉丝中奖记录
        }
    ...
    public function doWebDeletebranch() {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
            ...
            foreach ($doingslist as $doingslists) {
                //删除中奖记录
                //删除奖品第一步先恢复到奖池中
                $award = pdo_fetch("select id,prizetype,fansID from " . tablename('stonefish_bigwheel_award') . " where id = :id", array(':id' => $doingslists['prizeid']));
                $prize = pdo_fetch("select id,prizedraw from " . tablename('stonefish_bigwheel_prize') . " where id = :id", array(':id' => $award['prizetype']));
                pdo_update('stonefish_bigwheel_prize', array('prizedraw' => $prize['prizedraw'] - 1), array('id' => $award['prizetype']));
                //删除奖品第一步先恢复到奖池中
                //查询粉丝是否还有中奖记录，没有则需要改变粉丝状态
                $fansaward = pdo_fetch("select id from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and fansID = :fansID and uniacid=:uniacid and id!=:id", array(':rid' => $rid, ':fansID' => $award['fansID'], ':uniacid' => $_W['uniacid'], ':id' => $doingslists['prizeid']));
                if (empty($fansaward)) {
                    pdo_update('stonefish_bigwheel_fans', array('zhongjiang' => 0), array('fansID' => $award['fansID'], 'uniacid' => $_W['uniacid'], 'rid' => $rid));
                } else {
                    $awardnum = pdo_fetchcolumn("select count(*) from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and uniacid=:uniacid and fansID=:fansID and id!=:id", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':fansID' => $award['fansID'], ':id' => $doingslists['prizeid']));
                    pdo_update('stonefish_bigwheel_fans', array('awardnum' => $awardnum, 'zhongjiang' => 1), array('fansID' => $award['fansID'], 'uniacid' => $_W['uniacid'], 'rid' => $rid));
                }
                //查询粉丝是否还有中奖记录，没有则需要改变粉丝状态
                //查询奖品是否为虚拟积分，如果是则扣除相应的积分
    ...
    public function doWebAwardlist() {
        ...
        //查询是否有商户网点权限
        //所有奖品类别		
        $reply = pdo_fetch("select turntable from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $award = pdo_fetchall("select * from " . tablename('stonefish_bigwheel_prize') . " where rid = :rid and uniacid=:uniacid and turntable=:turntable order by `id` asc", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':turntable' => $reply['turntable']));
        foreach ($award as $k => $awards) {
            $award[$k]['num'] = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and uniacid=:uniacid and prizetype='" . $awards['id'] . "'", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        }
        //所有奖品类别
        //导出标题		
        if ($_GPC['status'] == 0) {
        ...
        $total = pdo_fetchcolumn("select count(a.id) from " . tablename('stonefish_bigwheel_award') . " a where a.rid = :rid and a.uniacid=:uniacid " . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 12;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select a.* from " . tablename('stonefish_bigwheel_award') . " a where a.rid = :rid and a.uniacid=:uniacid  " . $where . " order by a.id desc " . $limit, $params);
        //中奖资料
        foreach ($list as &$lists) {
            $lists['realname'] = pdo_fetchcolumn("select realname from " . tablename('stonefish_bigwheel_fans') . " where fansID = :fansID", array(':fansID' => $lists['fansID']));
            $lists['mobile'] = pdo_fetchcolumn("select mobile from " . tablename('stonefish_bigwheel_fans') . " where fansID = :fansID", array(':fansID' => $lists['fansID']));
            $lists['credit_type'] = pdo_fetchcolumn("select credit_type from " . tablename('stonefish_bigwheel_prize') . " where id = :id", array(':id' => $lists['prizetype']));
        }
        //中奖资料	
        //一些参数的显示
        $num1 = pdo_fetchcolumn("select total_num from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid", array(':rid' => $rid));
        $num2 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and status=1", array(':rid' => $rid));
        $num3 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and status=2", array(':rid' => $rid));
        $num4 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and status=0", array(':rid' => $rid));
        //一些参数的显示
    ...
    public function doWebSetstatus() {
        ...
        $temp = pdo_update('stonefish_bigwheel_award', $p, array('id' => $id));
        if ($temp == false) {
            message('抱歉，刚才操作数据失败！', '', 'error');
        } else {
            //修改用户状态
            if ($status >= 1) {
                $fansID = pdo_fetchcolumn("select fansID from " . tablename('stonefish_bigwheel_award') . " where id=:id", array(':id' => $id));
                $statusis = pdo_fetchcolumn("select status from " . tablename('stonefish_bigwheel_award') . " where fansID=:fansID and status>0 order by `status` asc", array(':fansID' => $fansID));
                pdo_update('stonefish_bigwheel_fans', array('zhongjiang' => $statusis), array('fansID' => $fansID));
            }
            //恢复奖品数量到奖池
            if ($status == 0) {
                $name = pdo_fetchcolumn("select prizetype from " . tablename('stonefish_bigwheel_award') . " where id=:id", array(':id' => $id));
                $prize = pdo_fetch("select * from " . tablename('stonefish_bigwheel_prize') . " where id = :id", array(':id' => $name));
                pdo_update('stonefish_bigwheel_prize', array('prizedraw' => $prize['prizedraw'] - 1), array('id' => $prize['id']));
                $awardfans = pdo_fetch("select awardnum,id from " . tablename('stonefish_bigwheel_fans') . " where rid=:rid and uniacid=:uniacid and fansID=:fansID", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':fansID' => $prize['fansID']));
                pdo_update('stonefish_bigwheel_fans', array('awardnum' => $awardfans['awardnum'] - 1), array('id' => $awardfans['id']));
            }
            //恢复奖品数量到奖池
            //从奖池减少奖品
            if ($status == 1) {
                ...
                if ($prize['prizetotal'] - $prize['prizedraw'] >= 1) {
                    ...
                    pdo_update('stonefish_bigwheel_award', array('status' => 0), array('id' => $id));
