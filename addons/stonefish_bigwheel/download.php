...
$reply = pdo_fetch("select isrealname,ismobile,isqq,isemail,isaddress,isgender,istelephone,isidcard,iscompany,isoccupation,isposition,isfansname from " . tablename('stonefish_bigwheel_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
...
if($fans=='branch'){
	...
	foreach ($list as &$row) {
		...
		$fansID = pdo_fetchcolumn("select fansID from " . tablename('stonefish_bigwheel_fans') . "  where mobile = :mobile and rid=:rid and uniacid=:uniacid", array(':mobile' => $row['mobile'],':rid' => $rid,':uniacid' => $_W['uniacid']));
		$row['awardinfo']='';
		$awards = pdo_fetchall("select name from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and fansID=:fansID", array(':rid' => $rid,':fansID' => $fansID));
		if(!empty($awards)){
	...
	foreach ($list as &$lists) {
		$lists['awardinfo']='';
		$awards = pdo_fetchall("select name from " . tablename('stonefish_bigwheel_award') . " where rid = :rid and fansID=:fansID", array(':rid' => $rid,':fansID' => $lists['fansID']));
		if(!empty($awards)){
			foreach ($awards as &$awardid) {
    ...
    $list = pdo_fetchall("select a.*,b.realname,b.mobile,b.qq,b.email,b.address,b.gender,b.telephone,b.idcard,b.company,b.occupation,b.position from ".tablename('stonefish_bigwheel_award')." as a  left join ".tablename('stonefish_bigwheel_fans')." 
