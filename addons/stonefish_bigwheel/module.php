...
class Stonefish_bigwheelModule extends WeModule {
    ...
    public function fieldsFormDisplay($rid = 0) {
        ...
        if (!$reply) {
            ...
            $reply = array(
				...
				"bigwheelpic" => "../addons/stonefish_bigwheel/template/images/activity-lottery-6.png",
				"bigwheelimg" => "../addons/stonefish_bigwheel/template/images/activity-inner.png",
				"bigwheelimgan" => "../addons/stonefish_bigwheel/template/images/activity.png",
				"bigwheelimgbg" => "../addons/stonefish_bigwheel/template/images/activity_bg.png",
		...
		if (!$share) {
			...
			foreach ($ids as $acid=>$idlists) {
                $share[$acid] = array(
				    ...
				    "share_picurl" => "../addons/stonefish_bigwheel/template/images/share.png",
				    "share_pic" => "../addons/stonefish_bigwheel/template/images/img_share.png",
				    "sharenumtype" => 0,
				    "sharenum" => 0,
					"sharetype" => 1,
				);
    ...
    public function fieldsFormSubmit($rid) {
        ...
        if (empty($id)) {
			...
			if ($token['config']){
                $id = pdo_insert('stonefish_bigwheel_reply', $insert);
			}
        } else {
            if ($token['config']){
    ...
    public function ruleDeleted($rid) {
        pdo_delete('stonefish_bigwheel_award', array('rid' => $rid));
        pdo_delete('stonefish_bigwheel_reply', array('rid' => $rid));
        pdo_delete('stonefish_bigwheel_fans', array('rid' => $rid));
		pdo_delete('stonefish_bigwheel_data', array('rid' => $rid));
