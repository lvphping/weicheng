...
class Coupon_EweiShopV2Page extends PluginWebPage 
{
	...
	public function add() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			{
				$data = array('title' => $title, 'type' => $type, 'mode' => 5, 'endtime' => $endtime, 'starttime' => $starttime, 'coupon' => json_encode($coupon), 'uniacid' => $_W['uniacid'], 'status' => 1, 'img' => '../addons/ewei_shopv2/plugin/exchange/static/img/exchange.jpg', 'title_reply' => '优惠券兑换', 'content' => '欢迎来到兑换中心,点击进入兑换', 'code_type' => intval($_GPC['code_type']), 'binding' => $binding);
	...
	public function edit() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($title)) && !(empty($endtime)) && !(empty($starttime))) 
			{
				$data = array('title' => $title, 'type' => $type, 'mode' => 5, 'endtime' => $endtime, 'starttime' => $starttime, 'coupon' => json_encode($coupon), 'uniacid' => $_W['uniacid'], 'status' => 1, 'code_type' => intval($_GPC['code_type']), 'binding' => $binding);
