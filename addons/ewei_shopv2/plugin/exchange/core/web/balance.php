...
class Balance_EweiShopV2Page extends PluginWebPage 
{
	...
	public function edit() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($title)) && !(empty($endtime)) && !(empty($type)) && !(empty($starttime))) 
			{
				$data = array('title' => $title, 'endtime' => $endtime, 'type' => $type, 'balance_left' => $left_rand, 'balance_right' => $right_rand, 'mode' => 2, 'status' => 1, 'uniacid' => $_W['uniacid'], 'status' => 1, 'balance' => $value, 'starttime' => $starttime, 'code_type' => intval($_GPC['code_type']), 'binding' => $binding);
