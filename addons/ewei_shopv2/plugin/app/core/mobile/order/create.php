...
class Create_EweiShopV2Page extends AppMobilePage 
{
	...
	public function main() 
	{
		...
		foreach ($goods as $g ) 
		{
			$allgoods[] = array('id' => $g['goodsid'], 'title' => $g['title'], 'thumb' => tomedia($g['thumb']), 'optionid' => (int) $g['optionid'], 'optiontitle' => $g['optiontitle'], 'hasdiscount' => empty($g['isnodiscount']) && !(empty($g['dflag'])), 'total' => $g['total'], 'price' => ($g['unitprice'] < $g['marketprice'] ? $g['marketprice'] : $g['unitprice']));
		}
		$newFields = array();
		...
		$result = array( 'member' => array('realname' => $member['realname'], 'mobile' => $member['carrier_mobile']), 'showTab' => (0 < count($carrier_list)) && !($isverify) && !($isvirtual), 'showAddress' => !($isverify) && !($isvirtual), 'carrierInfo' => (!(empty($carrier_list)) ? $carrier_list[0] : false), 'address' => $address, 'goods' => $allgoods, 'changenum' => $changenum, 'hasinvoice' => $hasinvoice, 'invoicename' => $invoicename, 'couponcount' => $couponcount, 'deductcredit' => $deductcredit, 'deductmoney' => $deductmoney, 'deductcredit2' => $deductcredit2, 'stores' => $stores, 'fields' => (!(empty($order_formInfo)) ? $newFields : false), 'f_data' => (!(empty($order_formInfo)) ? $f_data : false), 'dispatch_price' => $dispatch_price, 'goodsprice' => $goodsprice, 'taskdiscountprice' => $taskdiscountprice, 'discountprice' => $discountprice, 'isdiscountprice' => $isdiscountprice, 'showenough' => $saleset['showenough'], 'enoughmoney' => $saleset['enoughmoney'], 'enoughdeduct' => $saleset['enoughdeduct'], 'merch_showenough' => $merch_saleset['merch_showenough'], 'merch_enoughmoney' => $merch_saleset['merch_enoughmoney'], 'merch_enoughdeduct' => $merch_saleset['merch_enoughdeduct'], 'merchs' => $merchs, 'realprice' => $realprice );
		app_json($result);
	}
