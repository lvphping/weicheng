...
class Clearing_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			$data = array('merchid' => $merchid, 'clearno' => (empty($_GPC['clearno']) ? m('common')->createNO('merch_clearing', 'clearno', 'MB') : $_GPC['clearno']), 'uniacid' => $_W['uniacid'], 'goodsprice' => floatval($_GPC['goodsprice']), 'dispatchprice' => floatval($_GPC['dispatchprice']), 'deductprice' => floatval($_GPC['deductprice']), 'deductcredit2' => floatval($_GPC['deductcredit2']), 'discountprice' => floatval($_GPC['discountprice']), 'isdiscountprice' => floatval($_GPC['isdiscountprice']), 'price' => floatval($_GPC['price']), 'realprice' => floatval($_GPC['realprice']), 'payrate' => floatval($_GPC['payrate']), 'realpricerate' => floatval($_GPC['realpricerate']), 'deductenough' => floatval($_GPC['deductenough']), 'merchdeductenough' => floatval($_GPC['merchdeductenough']), 'status' => intval($_GPC['status']), 'finalprice' => floatval($_GPC['finalprice']), 'remark' => $_GPC['remark'], 'starttime' => strtotime($_GPC['datetime']['start']), 'endtime' => strtotime($_GPC['datetime']['end']), 'createtime' => time());
			($data['realprice'] < ($data['goodsprice'] + $data['dispatchprice']) ? $data['realprice'] = ($data['goodsprice'] + $data['dispatchprice']) - $data['merchdeductenough'] : $data['realprice']);
